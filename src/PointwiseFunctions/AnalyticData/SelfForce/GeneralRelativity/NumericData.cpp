...
Interpolator1D load_data_from_file_1D(const std::string& filename,
  ...
  for (size_t i = 0; i < num_points; ++i) {
    coord[i] = coord_min + i * delta_coord;
  }
...
NumericData::variables(
  ...
  for (size_t i = 0; i < num_points; ++i) {
    ...
    for (size_t k = 0; k < 10; ++k) {
      src_re_arr[k] = interpolator.interpolate(weights, 2 * k);
      src_im_arr[k] = interpolator.interpolate(weights, 2 * k + 1);
      if (pi_2_rotation_) {
        const std::complex<double> rotated =
            (src_re_arr[k] + std::complex<double>(0., 1.) * src_im_arr[k]) *
            (2. * M_PI * rotation[i]);
  ...
  // DEBUG: evaluate Seff at fixed reference point for comparison with
  // CircularOrbit. The Seff interpolator was built with actual theta values
  // (not cos_theta), so we must pass theta_ref to get_weights and cos_ref only
  // to convert_Seff_vr.
  // {
  ...
  //         M_PI_2 - 0.02;  // actual theta for interpolator lookup
  //     const double cos_ref =
  //         cos(theta_ref);  // cos(theta) for convert_effsource_Seff_vr
  //     const double r_c = std::clamp(r_ref, interpolator.lower_bound(0),
  //                                   interpolator.upper_bound(0));
  //     const double theta_c = std::clamp(theta_ref,
  //     interpolator.lower_bound(1),
  //                                       interpolator.upper_bound(1));
  //     const auto ref_weights = interpolator.get_weights(r_c, theta_c);
  //     std::array<double, 10> ref_re{}, ref_im{}, ref_conv_re{},
  //     ref_conv_im{}; const double delta_phi_pt = m_mode_number_ * a / (r_plus
  //     - r_minus) *
  //                                 log((r_ref - r_plus) / (r_ref - r_minus));
  //     const std::complex<double> rotation_pt =
  //         cos(delta_phi_pt) + std::complex<double>(0., 1.) *
  //         sin(delta_phi_pt);

  //     for (size_t k = 0; k < 10; ++k) {
  //       ref_re[k] = interpolator.interpolate(ref_weights, 2 * k);
  //       ref_im[k] = interpolator.interpolate(ref_weights, 2 * k + 1);
  //       if (pi_2_rotation_) {
  //         const std::complex<double> rotated = (ref_re[k]+
  //         std::complex<double>(0.,1.) * ref_im[k]) * (2. * M_PI *
  //         rotation_pt); ref_re[k] = rotated.real(); ref_im[k] =
  //         rotated.imag();
  //       }
  //     }
  //     detail::convert_effsource_Seff_vr(m_mode_number_, a, r_ref, cos_ref,
  //                                       ref_re, ref_im, ref_conv_re,
  //                                       ref_conv_im);
  //     const size_t comp_tt = tnsr::aa<ComplexDataVector,
  //     3>::get_storage_index(
  //         std::array<size_t, 2>{{0, 0}});
  //     Parallel::printf(
  //         "NumericData Seff @ r=7.8 theta=pi/2-0.02: eff_tt=(%.8e,%.8e)\n",
  //         ref_conv_re[comp_tt], ref_conv_im[comp_tt]);
  //   }
  // }
  if (field_is_regularized) {
