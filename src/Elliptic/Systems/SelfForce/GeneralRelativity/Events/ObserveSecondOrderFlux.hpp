...
class ObserveSecondOrderFlux : public Event {
 public:
  struct FirstOrderDataDirectory {
    using type = std::string;
    static constexpr Options::String help = {
        "The path to the first order volume data."};
  };
  using BackgroundTag =
  ...
  explicit ObserveSecondOrderFlux(CkMigrateMessage* msg) : Event(msg) {}
  using PUP::able::register_constructor;
  WRAPPED_PUPable_decl_template(ObserveSecondOrderFlux);  // NOLINT

  using options = tmpl::list<FirstOrderDataDirectory>;

  static constexpr Options::String help =
      "Observe the energy flux at the outer boundary.";

  ObserveSecondOrderFlux() = default;
  explicit ObserveSecondOrderFlux(std::string first_order_data_directory)
      : first_order_data_directory_(std::move(first_order_data_directory)) {
    detail::ensure_first_order_interpolator_loaded(first_order_data_directory_);
  }
  ...
  void operator()(const ObservationBox<ComputeTagsList, DataBoxType>& box,
    ...
    if (element.external_boundaries().contains(direction)) {
      ...
      std::tie(energy_flux, surface_area) = detail::extract_second_order_flux(
    ...
    const std::string subfile_name = "SecondOrderFlux";
  ...
  get_observation_type_and_key_for_registration(
    ...
    return {
        {observers::TypeOfObservation::Reduction,
         observers::ObservationKey("SecondOrderFlux" +
  ...
  void pup(PUP::er& p) override {
    Event::pup(p);
    p | first_order_data_directory_;
  }

 private:
  std::string first_order_data_directory_;
};

/// \cond
// NOLINTBEGIN(cppcoreguidelines-avoid-non-const-global-variables)
template <typename ArraySectionIdTag>
PUP::able::PUP_ID ObserveSecondOrderFlux<ArraySectionIdTag>::my_PUP_ID = 0;
